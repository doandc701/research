<template>
  <div>
    ...
    <div v-loading="yourNFTLoading" class="account-nfts-container">
      ...
      <!-- Sort, Filters -->
      <div class="button-transfer-group">
        ...
        <sort-filters :loading="sortFiltersLoading" :location="collection.location" @sortFiltersSubmit="sortFiltersSubmit" />
      </div>
      ...
      <pagination v-show="totalNFT>0" :total="totalNFT" :page.sync="NFTQuery.page" :limit.sync="NFTQuery.limit" :page-sizes="NFTQuery.pageSizes" @pagination="handleLoadPageNTF" />
    </div>
    <!-- end: Your NFTs -->
          ...
          <div class="panel-group">
            <div class="card-panel">
              <div class="card-panel-icon-wrapper">
                <el-image :src="VNFTData.img" class="card-panel-icon" style="width:auto;" fit="cover" lazy>
                  <div slot="placeholder" class="image-slot">
                    <i class="el-icon-loading" />
                  </div>
                </el-image>
              </div>
              <div class="card-panel-description" style="margin-bottom:10px;">
...
const filterType = (collected, types) => collected.filter((item) => types.indexOf(item.type) !== -1)
const filterCount = (collected, counts) => collected.filter((item) => counts.indexOf(item.count) !== -1)
const filterAttr = (collected, attrs) => collected.filter((item) => checkIfDuplicateExists(attrs.concat(item.accessories)))

export default {
  name: 'MyProfileCollected',
  ...
  created() {
    ...
    // this.address = '0x31C71f4Cd01FDDD940A46DAfd72d3291f52040a4' // test [VPunks auction V1]
    // this.address = '0x2011c0323c95ecce76c2ad00fbc3be57d556f81c' // test [Milk]
    ...
    // this.appendRunningAucServer(true, false)
  },
  mounted() {
  ...
  methods: {
    ...
    // Your NFTs -------------------------------------------------------
    // @
    loadPageYourNFT(queryObject) {
      if (this.ownershipNTFList.length === 0 || queryObject) {
        this.appendYourNFT(true, queryObject)
      } else {
        this.handleLoadPageNTF()
      }
    },
    // @
    async getTotalNFT() {
    ...
    // @
    async appendYourNFT(getTotal, queryObject) {
      if (queryObject) {
      ...
      // var newSort = ''
      const indexs = []
      for (let i = 0; i < this.totalNFT; i++) indexs.push(i)
      await this.$store.getters.instances.ERC721PeripheryV1.tokenOfOwnerByIndexs(this.$store.getters.instances.NFTContract.address, this.address, indexs).then(results => {
        ...
        this.ownershipNTFList = []
        for (let i = 0; i < results[0].length; i++) {
          const item = this.getItemInfos(results[0][i])
          this.ownershipNTFList.push(item)
        }
        if (!queryObject) {
          this.handleLoadPageNTF()
          return
        ...
        let newList = this.ownershipNTFList
        this.queryObject = queryObject

        const order = queryObject.order
        const typeArr = queryObject.type
        const countArr = queryObject.count
        const attrArr = queryObject.attr
        ...
        newList = newList.sort(newcompare)
        // console.log('newList:', newList)
        this.ownershipNTFList = newList
        this.handleLoadPageNTF()
      }).catch(() => {})
      // this.handleLoadPageNTF()
      // const gg = this.newSort
      // //   this.handleLoadPageNTF(gg)
      //   console.log('fdgsdg',this.handleLoadPageNTF(gg))
    },
    handleLoadPageNTF() {
      this.totalNFT = this.ownershipNTFList.length
      // this.totalNFT = leng
      // leng = 0 ? this.totalNFT : this.totalNFT = leng
      ...
      for (let i = start; i <= end; i++) {
        this.yourNFTList.push(this.ownershipNTFList[i])
      }
      this.appendYourNFTDone()
    },
    // @
    ...
    // SortFilters -------------------------------------------------------
    sortFiltersSubmit(queryString, queryObject) { // queryString, queryObject
      this.loadPageYourNFT(queryObject)
    },
    // end: SortFilters -------------------------------------------------------
...
<!-- vấn đề
Lưu lại ds trả về từ result
Dùng fillter sort tạo một danh sách mới
Dùng ds đã được sort đó chia page -->
