  ...
  formEl.validate(async (valid: any) => {
    if (valid) {
      ...
      if (labelText.value == "Add") {
        const findAlike = apiDataOriginal.value.find(
          (item: Messages) =>
            item.msg.toLowerCase() ==
            numberValidateForm.msg.trim().toLowerCase()
        );
        if (findAlike) {
          ElMessage.error(
            `Oops, message ${numberValidateForm.msg} already exists.`
          );
          cusDisable.value = true;
          formEl.resetFields();
          return;
        ...
        if (typeEdit.value == "original") {
          ...
          if (Object.entries(data).length === 0) {
            ElMessage.error("Oops, message already exists.");
            return;
          }
...
function handleCancel(formEl: any | undefined) {
  labelText.value = "Add";
  cusDisable.value = true;
  formEl.resetFields();
  ElMessage({
...
<template>
  ...
  >
    ...
    <el-form-item>
      ...
      >
      <el-button
        v-if="labelText == 'Update'"
        type="danger"
        @click="handleCancel(formRef)"
        >Cancel</el-button
      >
      <el-button type="danger" @click="handleRandomMessages"
        >Random Messages</el-button
      >
    </el-form-item>
  </el-form>
  <div>
