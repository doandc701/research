...
const usernameRule = [
  {
    validator: (rule, value, callback) => {
      ...
      const REGEXP_USERNAME = /^[0-9A-Za-z\.]+$/; // a-z A-Z 0-9 ".", không chứa ký tự đặc biệt
      const REGEXP_CONTAINS_LETTER = /[a-zA-Z]/; // chứa chữ cái a-z hoặc A-Z
      if (value === "") {
      ...
      if (!REGEXP_CONTAINS_LETTER.test(value)) {
        callback(
          new Error(
            "Sorry, usernames must include at least one alphabetical character (a-z)"
          )
        );
      } else {
        callback();
    ...
    <Motion :delay="300">
      <div class="ask" style="padding: 0px 0px">
        ...
        <p>
          ...
          <el-link @click="onBack">Login</el-link>
        </p>
      </div>
    </Motion>
  </el-form>
